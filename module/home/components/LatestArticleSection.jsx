...
async function getLatestArticles() {
  ...
  const res = await fetch(
    `${process.env.API_URL}/posts?populate=*&sort=updatedAt:desc&pagination[page]=1&pagination[pageSize]=3`,
    { cache: "no-store" }
  );
  ...
  return (
    <section className="w-full px-5 md:px-12 lg:px-20 xl:px-24 py-20 md:py-28">
      <div className="flex items-center justify-between">
        <h2 className="text-[24px] md:text-[28px] font-medium text-[#038F7A]">
          Artikel terbaru
        </h2>
        <Link
          href="/blog/semua"
          className="text-[14px] md:text-[16px] text-[#038F7A] underline underline-offset-4"
        >
          Lihat Selengkapnya
        </Link>
      </div>
