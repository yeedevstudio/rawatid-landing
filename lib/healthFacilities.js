...
// Kolom provinceId & cityId di tabel health_facilities null untuk SEMUA baris,
// jadi filter wilayah tidak bisa membandingkan id ke id. Yang tersedia di data
// faskes cuma provinceCode/cityCode. Peta ini menerjemahkan id master wilayah
// (nilai yang dikirim dropdown) ke kodenya, lalu kode itulah yang dicocokkan.
//
// Bentuknya: Map("1" -> "11") untuk provinsi, Map("1" -> "1101") untuk kota —
// kode master "11.01" disamakan ke bentuk tanpa titik seperti di data faskes.
const codeById = (rows) =>
  new Map(rows.filter((r) => r.id != null && r.code).map((r) => [String(r.id), digits(r.code)]));

// Filter wilayah: key yang dikirim client -> field baris yang dibandingkan.
const WILAYAH_FILTERS = {
  provinceId: { field: "provinceCode", map: "provinceCodeById" },
  cityId: { field: "cityCode", map: "cityCodeById" },
};
...
// Filter yang membandingkan ID, bukan string nama. province & city tetap
// berbasis nama karena hasil resolve dari master wilayah.
...
let cache = { rows: null, masters: null, wilayah: null, at: 0, inflight: null };
...
async function fetchAll() {
  ...
  const provinceRows = Array.isArray(provincesJson?.data) ? provincesJson.data : [];
  const cityRows = Array.isArray(citiesJson?.data) ? citiesJson.data : [];
  const provinces = indexBy(provinceRows);
  const cities = indexBy(cityRows);
  const maps = labelMaps(masters);
  const wilayah = {
    provinceCodeById: codeById(provinceRows),
    cityCodeById: codeById(cityRows),
  };
  ...
  const rows = raw
    .map((r) => {
      ...
      return {
        ...
        provinceCode: str(r.provinceCode),
  ...
  return { rows, masters, wilayah };
}

async function loadAll() {
  const now = Date.now();
  if (cache.rows && now - cache.at < FACILITIES_TTL_MS) {
    return { rows: cache.rows, masters: cache.masters, wilayah: cache.wilayah };
  }
...
// /api/health-facilities/list. Semuanya berbasis ID, bukan nama.
export const FILTER_KEYS = [...Object.keys(WILAYAH_FILTERS), ...ID_FILTER_KEYS];
...
export async function getFacilities({
  ...
  const { rows: all, wilayah } = await loadAll();

  const q = String(search || "").trim().toLowerCase();

  // Semua filter diringkas jadi pasangan [field baris, nilai yang dicari].
  // provinceId/cityId diterjemahkan dulu ke kode lewat master wilayah, karena
  // data faskes tidak punya kolom id-nya (lihat WILAYAH_FILTERS). ID yang tidak
  // dikenal menghasilkan kode kosong -> tidak ada yang cocok, dan itu benar.
  const active = [];
  for (const key of FILTER_KEYS) {
    const value = str(filters[key]);
    if (!value) continue;
    const w = WILAYAH_FILTERS[key];
    active.push(w ? [w.field, wilayah[w.map].get(value) || ""] : [key, value]);
  }

  const matched = all.filter((f) => {
    if (q && !f.name.toLowerCase().includes(q)) return false;
    return active.every(([field, value]) => str(f[field]) === value);
  });
